function [Lint,lambda,lambda1,EpsNew]=ZeroCrossings(U,Fs,m_data,nu,low_freq,x_achse_k_spektrum_2pi,E_k)
if low_freq== Fs/2
    fc  = low_freq-1;
else
    fc  = low_freq;
...
[ns,l,sigmavoro]    = computeZeros(U,Fs,FilterSizes,m_data);
...
ylabel('$n_s \bar{l}$','FontName','times');
...
[EpsNew]        = PostProcEps(U,m_data,Fs,nu,x_achse_k_spektrum_2pi,E_k);
end


function [ns,l,sigmavoro]=computeZeros(U,Fs,FilterSizes,m_data)
for j=1:numel(FilterSizes)
    ...
    S           = sign(sU);
    I           = find((abs(diff(S)))>1);
    ns(j)       = numel(I)./numel(U).*Fs./m_data;
...
function [EpsNew]=PostProcEps(U,m_data,Fs,nu,x_achse_k_spektrum_2pi,E_k)
% UU              = m_data;
% UUstd           = std(U);
% U_tmp1          = m_data; 
% Fs              = Fs.*2.*pi./m_data;
...
% Nfft            = 4096*4;
% [P,f]           = pwelch(U-m_data,hanning(Nfft),Nfft/4,Nfft,(Fs.*2.*pi./m_data));
...
%     xlim([1 numel(P)]
    ...
    while b==1
        ...
        waitfor(msgbox('Use interactive data cursor to select the fit range of wave numbers (see readme).'));
