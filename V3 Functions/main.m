...
[f, E_f, f_avg, E_avg, P_avg] = spectrum(data,Fs,increment_bin);
...
if filter==1
    ...
    [data_filter,k_avg,Ek_avg,r_avg,Dr_avg,Dk_avg] = frequency_filter(data,Fs,low_freq,kin_vis,filter,save_path); 
else
	data_filter = data;
    low_freq    = Fs/2;
    close
...
if flip_data==1
    ...
    [data_filter,siginf,m_data] = normalization(data_filter,askYesno('Do you want to flip hotwire data?', 'Yes'));
    close
else
    [data_filter,siginf,m_data] = normalization(data_filter,0);
end
if ischar(save_path)
    tmp_bar=bar.Position;
    close(bar)
...
[r_struc,S_exp_2,S_exp_3,S_exp_4,S_exp_5,S_exp_6,S_exp_7]=plot_struc_function(askInput({sprintf('Number of seperated scales between Integral and Taylor (calculation of structure functions):')}, {num2str(30,'%1.0f')}),Fs,data_filter,m_data,int_L,taylor_L,save_path);
...
markov=wilcoxon_test(data_filter,Fs,m_data,int_L,taylor_L,diss_scale,increment_bin,save_path,f_avg, E_avg);
...
conditional_PDF_markov(data_filter,Fs,m_data,markov,askInput({sprintf('Increment of the second condition:')}, {'0'}),increment_bin,min_events,save_path)
...
[evaluated] = KM_Calculation(increment_bin,min_events,evaluated,step_con_moment,Fs,taylor_L,m_data,multi_point,condition);
if ischar(save_path)
    tmp_bar=bar.Position;
    close(bar)
    save(fullfile(save_path,'evaluated.mat'),'-v7.3') 
...
KM_plot_raw(evaluated,Fs,taylor_L,multi_point,condition,save_path)
...
if test_opti==1   
    ...
    [evaluated] = KM_STP_optimization(evaluated,increment_bin,Fs,markov,data_filter,m_data,taylor_L,test_opti,multi_point,condition,tol,min_events,tol_opti,save_path);
    ...
    close all
    [evaluated] = KM_STP_optimization(evaluated,increment_bin,Fs,markov,data_filter,m_data,taylor_L,0,multi_point,condition,tol,min_events,tol_opti,save_path);
else
    tol_opti = askInput({sprintf('Optimization: Tolerance of the range of Kramers-Moyal coefficients in %%')}, {'10'})/100;
    [evaluated] = KM_STP_optimization(evaluated,increment_bin,Fs,markov,data_filter,m_data,taylor_L,0,multi_point,condition,tol,min_events,tol_opti,save_path);
end
...
[co_KM_opti,co_KM_opti_no_offset,co_KM_non_opti,fitresult_D1_conf,fitresult_D2_conf] = FIT_KM(evaluated,increment_bin,taylor_L,Fs,m_data,markov,multi_point,condition,save_path);
if ischar(save_path)
    tmp_bar=bar.Position;
    close(bar)
    save(fullfile(save_path,'evaluated.mat'),'-v7.3') 
...
KM_plot(co_KM_opti,evaluated,Fs,taylor_L,multi_point,condition,save_path)
...
if multi_point==1
    CO_plot(co_KM_opti.(sprintf('point_eval_%d', askInput({sprintf('Plot: Multi-point number of point condition:')}, {num2str(ceil(length(condition)*1/4))}))), ...
            ...
            evaluated.point_eval_1,taylor_L);
else
    CO_plot(co_KM_opti,co_KM_non_opti,evaluated,taylor_L);
end
...
[Sm,Ds,DS,r,ind_trajec,rind,dr,r_s,~,~,A,~,~,~,~,~,~] = checkFT(evaluated,int_L,taylor_L,Fs,data_filter,m_data,z,co_KM_opti,data_length,markov,trajec,dr_ind);
...
if opti_IFT==1
    ...
    [co_IFT_opti,history]=OPTI_IFT_dij(tol_D1,tol_D2,evaluated,co_KM_opti,fitresult_D1_conf,fitresult_D2_conf,int_L,taylor_L,m_data,Fs,data_filter,z,iter,markov,trajec,dr_ind,save_path);
    ...
    % KM_plot(co_IFT_opti,evaluated,Fs,taylor_L,save_path)
    % CO_plot(co_IFT_opti,co_KM_opti,evaluated,taylor_L,co_KM_non_opti)
    if ischar(save_path)
        close all
        tmp_bar=bar.Position;
        close(bar)
    ...
    z               = 1;
    data_length     = 1;
    [Sm,Ds,DS,r,ind_trajec,rind,dr,r_s,~,~,A,~,~,~,~,~,~] = checkFT(evaluated,int_L,taylor_L,Fs,data_filter,m_data,z,co_IFT_opti,data_length,markov,trajec,dr_ind);
    plot_entropy(Sm,Ds,DS,increment_bin,save_path)
    if ischar(save_path)
        tmp_bar=bar.Position;
        close(bar)
