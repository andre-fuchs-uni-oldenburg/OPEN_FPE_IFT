...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
function [evaluated] = KM_STP_optimization(evaluated,increment_bin,Fs,markov,data,m_data,taylor_L,test_opti,multi_point,condition,tol,min_events,tol_opti,norm_ur,norm_r,save_path)
if multi_point==1
    if test_opti==1
        ...
        if sum(evaluated.(my_field)(scal).x_bin_not_nan>0)>0
            [evaluated.(my_field)] = pointwise_KM_STP_optimization(evaluated.(my_field),scal,increment_bin,markov,data,Fs,tol_opti,m_data,taylor_L,test_opti,multi_point,condition,tol,min_events,k,norm_ur,norm_r,save_path); 
        else
    ...
    if test_opti==1
        scal        = askInput({sprintf('Which scale number should be optimized?')}, {'1'});  
        [evaluated] = pointwise_KM_STP_optimization(evaluated,scal,increment_bin,markov,data,Fs,tol_opti,m_data,taylor_L,test_opti,multi_point,condition,tol,min_events,k,norm_ur,norm_r,save_path);   
    else
        f = uifigure;
