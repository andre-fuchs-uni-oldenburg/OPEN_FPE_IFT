...
% incr1 = Velocity increment for step tau1 when incr3>-tol+second_condi & incr3<tol+second_condi
% incr2 = Velocity increment for step tau2 when incr3>-tol+second_condi & incr3<tol+second_condi
...
%
% Arguments OUT
% P_AIB = PDF of A i.e incr1 conditioned on B i.e incr2 and when incr3>-tol+second_condi & incr3<tol+second_condi
% P_BIA = PDF of B i.e incr2 conditioned on A i.e incr1 and when incr3>-tol+second_condi & incr3<tol+second_condi
% P_AnB = Joint PDF of A i.e. incr1 and B i.e incr2 and when incr3>-tol+second_condi & incr3<tol+second_condi
% P_A = PDF of A i.e incr1
% P_B = PDF of B i.e incr2
% binP_AIB = 2D binning 
% x_mean_bin_2 = 1D array of centers of bins of incr2
% y_mean_bin_2 = 1D array of centers of bins of incr1
% events = number of elements that fall at the center in each bin
% counter_A = histogram of A
% counter_B = histogram of B
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [P_AIB,P_BIA,P_AnB,P_A,P_B,binP_AIB,x_mean_bin_2,y_mean_bin_2,events,counter_A,counter_B] = distribution_double(tau1,tau2,incr1,incr2,num_bin,dx,dy,x,y,dA,varargin)
...
if nargin <11
    ...
    %%%%%%%%%%%%%%%%%%%%% same as distribution %%%%%%%%%%%%%%%%%%%%%
        x_bin           = linspace(min(x),max(x),num_bin+1);
        y_bin           = linspace(min(y),max(y),num_bin+1);
    %%%%%%%%%%%%%%%%%%%%% same as distribution %%%%%%%%%%%%%%%%%%%%%
    %     if length(x_bin)==aufl+1 & length(y_bin)==aufl+1
            x_mean_bin_2(1:num_bin) = nan;
            y_mean_bin_2(1:num_bin) = nan;
            events(1:num_bin)=0;
    ...
    %         end
             ...
             for q=1:num_bin
                if q<num_bin     
                  x_mean_bin_2(q)   = nanmean(tmp1(tmp1>=x_bin(q) & tmp1<x_bin(q+1)));
    ...
    %               events(q)=length(tmp1(tmp1>=x_bin(q) & tmp1<x_bin(q+1)));
                  ...
                  y_mean_bin_2(q)   = nanmean(tmp2(index));
                  events(q)         = sum(index);
                else           
                  x_mean_bin_2(q)   = nanmean(tmp1(tmp1>=x_bin(q) & tmp1<=x_bin(q+1)));
    %               y_mean_bin(q)=nanmean(tmp2(tmp2>=y_bin(q) & tmp2<=y_bin(q+1)));
    ...
    for s=1:num_bin
        for q=1:num_bin
            binP_AIB(q,s)       = x_mean_bin_2(q)-y_mean_bin_2(s);
