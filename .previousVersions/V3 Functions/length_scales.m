...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
function [int_L,taylor_L,int_L_calc, taylor_L_calc,epsi, epsi_calc,diss_scale,Ce, Ce_calc, Re, Re_lambda]=length_scales(data,Fs,low_freq,kin_vis,C2,increment_bin,m_data,save_path)
...
% x_achse_spektrum_meter=m_data./x_achse_f_spektrum;
...
f_start     = askInput({sprintf('Lowest frequency of constant range in Hz:')}, {num2str(I(1),'%.3f')});
f_end       = askInput({sprintf('Highest frequency of constant range in Hz:')}, {num2str(I(2),'%.3f')});
vline(f_start,'k')              % Construct a verticle line at f_start
vline(f_end,'k')                % Construct a verticle line at f_end
f_start     = find(f>f_start, 1,'first');
f_end       = find(f<=f_end, 1,'last');
...
delete(tmp_ui);
[int_L_zero_cros,lambda_zero_cros,lambda1_zero_cros,Eps_zero_cros]=ZeroCrossings(data,Fs,m_data,kin_vis,low_freq,x_achse_k_spektrum_2pi,E_k);
...
set(gcf, 'Color', 'w')
legend('$E(k)$','$C_k\langle \epsilon \rangle^{2/3}(k-k_0)^{-5/3}$','$C_k\langle \epsilon \rangle^{2/3}k^{-5/3}$','Location','south')
fig_setup
set(gca,'XTick',[10^-2 10^0  10^2  10^4]);
fitresult;
fitresult2;
...
epsi_calc   = [epsi_S2,epsi_S3, (15*kin_vis*(uprimesquared/taylor_L^2)), fitresult.b, fitresult2.b, Eps_zero_cros];
...
if ischar(save_path)
    savefig(h,fullfile(save_path,'length_scales.fig'),'compact')
    for a = 1:length(h)
       exportgraphics(h(a),fullfile(save_path,sprintf('length_scales_%d.png', a)))
    end
end
close all
end